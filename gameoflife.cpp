...
void gameOfLife(size_t grid_size = 10);
std::vector<std::vector<bool>> generateGrid(size_t grid_size = 10);
...
int main(){
    size_t size;
    std::cout << "Welcome to the game of life!\n" << "How large do you want the grid to be (nxn): " << std::endl;
    std::cin >> size;
    gameOfLife(size);
}

std::vector<std::vector<bool>> generateGrid(size_t grid_size){
    ...
    std::cout << "Returning a grid of size = (" << grid_size << ',' << grid_size << ')' << std::endl;
    return grid; 
    
}

void gameOfLife(size_t grid_size){
    std::vector<std::vector<bool>> old_grid = generateGrid(grid_size);
